...
class MainWindow: NSWindowController {
    ...
    public func paint(scale: CGFloat) {
            if let screenSize = window?.screen?.frame {
                if (side == "left") {
                    window!.setFrame(NSRect(x: 0, y: 0, width: BASE * scale, height: screenSize.maxY), display: true)
                } else {
                    window!.setFrame(NSRect(x: screenSize.maxX - (BASE * scale), y: 0, width: BASE * scale, height: screenSize.maxY), display: true)
                }
            }
        }
    
    public func changeColor(temp: Int) {
        if let t = kelvin_table[temp] {
            let color = NSColor.fromRGB(red: Double(t.0), green: Double(t.1), blue: Double(t.2))
